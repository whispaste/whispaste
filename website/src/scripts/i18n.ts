...
export const i18n: Record<string, Record<string, string>> = {
  en: {
    ...
    'meta.description.download': 'Download WhisPaste for Windows, macOS, and Linux: via Microsoft Store, DMG, AppImage, .deb, or free on GitHub. Offline-first voice input, no account, no subscription.',
    'meta.title.privacy': 'WhisPaste Privacy Policy',
    'meta.description.privacy': 'Privacy Policy for the WhisPaste desktop app: how it handles your data, offline by default, no account, transcripts stay on your machine, only anonymous opt-out usage stats.',
    'meta.title.screenshots': 'WhisPaste Screenshots: desktop voice input',
    'meta.description.screenshots': 'See WhisPaste in action: workspace, detail editor, Voice Snippets, settings, and analytics in light and dark theme. Desktop voice input for Windows, macOS & Linux.',
    'meta.title.impressum': 'WhisPaste Legal Notice',
    ...
    'meta.title.changelog': 'WhisPaste Changelog',
    ...
    'faq.sysreq.a': 'Minimum: Windows\u00a010 (64-bit) or macOS\u00a010.15 Catalina\u202f+\u00a0\u00b7 8\u202fGB RAM (required, enforced at startup)\u00a0\u00b7 1\u2013\u20132\u202fGB free disk space. No GPU required. WhisPaste always falls back to CPU, and the Parakeet engine is CPU-only by design and still several times faster than Whisper on CPU. Recommended (Whisper engine): 16\u202fGB RAM\u00a0\u00b7 dedicated GPU with 2\u2013\u20134\u202fGB VRAM (NVIDIA CUDA, AMD or Intel Vulkan) for 5\u202f\u00d7 faster transcription. GPU VRAM by model: compact ~300\u202fMB \u00b7 balanced ~900\u202fMB \u00b7 premium (turbo) ~2.6\u202fGB \u00b7 ultra ~3.6\u202fGB. Apple Silicon: unified memory counts for both CPU and GPU, so 8\u202fGB covers the balanced model.',
    ...
    // the mouse.
    'useCaseHub.seoTitle': 'WhisPaste use cases: voice input for developers, students, and everyday writing',
    ...
    'useCase.students.seoTitle': 'WhisPaste for students & researchers: speak your notes and essays instead of typing',
    ...
    'explainer.whisperDesktop.faq.oldGpu.a': 'Yes. WhisPaste uses CUDA on NVIDIA GPUs, Vulkan on AMD/Intel/older GPUs, and falls back to the CPU when no compatible GPU is available — so it runs on a wide range of hardware.',
    ...
    'explainer.engines.seoTitle': 'WhisPaste: Whisper vs. Parakeet, which local speech engine should you use?',
    'explainer.engines.seoDescription': "Compare WhisPaste's two on-device speech engines: Whisper (whisper.cpp, 99 languages, GPU-accelerated) and Parakeet (NVIDIA Parakeet-TDT, CPU-only, several times faster). Free and open source.",
    ...
    'explainer.engines.section.whisper.body': "Whisper runs as whisper.cpp, compiled directly into WhisPaste via dart:ffi: no subprocess, no server. It covers 99 languages and five model sizes (tiny, base, small, medium, large-v3), so you can trade download size and speed for accuracy. On supported hardware it accelerates via CUDA on NVIDIA GPUs or Vulkan on any modern GPU, and falls back to CPU automatically if a GPU crashes or is unavailable.",
    ...
    'engines.whisper.desc': 'Runs on CUDA, Vulkan, or plain CPU, with five model sizes to balance speed and accuracy, from tiny to large-v3.',
  ...
  de: {
    ...
    'meta.description.download': 'WhisPaste für Windows, macOS und Linux herunterladen: über den Microsoft Store, als DMG, AppImage, .deb oder kostenlos von GitHub. Offline-first, ohne Account, ohne Abo.',
    'meta.title.privacy': 'Datenschutz: WhisPaste',
    'meta.description.privacy': 'Datenschutzerklärung für WhisPaste, wie Website und Desktop-App mit deinen Daten umgehen: offline als Standard, kein Account, Transkripte bleiben lokal, nur anonyme, abschaltbare Nutzungsstatistik.',
    ...
    'useCaseHub.seoTitle': 'WhisPaste Anwendungsfälle: Spracheingabe für Entwickler, Studierende und den Alltag',
    ...
    'useCase.everyday.seoTitle': 'WhisPaste für Vielschreiber: E-Mails und Nachrichten einfach sprechen',
    ...
    'useCase.students.seoTitle': 'WhisPaste für Studierende & Forschende: Notizen und Texte sprechen statt tippen',
    ...
    'explainer.whisperDesktop.faq.oldGpu.a': 'Ja. WhisPaste nutzt CUDA auf NVIDIA-GPUs, Vulkan auf AMD/Intel/älteren GPUs und fällt auf die CPU zurück, wenn keine kompatible GPU verfügbar ist — läuft also auf breiter Hardware.',
    ...
    'explainer.whisperDesktop.section.performance.body': 'whisper.cpp läuft auf jeder unterstützten Maschine auf reiner CPU (Windows 10+, macOS 10.15+, Ubuntu Linux), deshalb genügt ein 8-GB-Laptop ohne dedizierte GPU für das kompakte Modell. Auf einem Rechner mit dedizierter GPU (NVIDIA CUDA, AMD oder Intel Vulkan) entsteht dasselbe Transkript ungefähr fünfmal schneller. Apple Silicon nutzt Unified Memory, weshalb 8 GB bereits für das ausgewogene Modell reichen. Gar keine GPU vorhanden? WhisPastes zweite On-Device-Engine, Parakeet, ist bewusst CPU-first gebaut und bleibt auf derselben Hardware mehrfach schneller als Whisper, auf Kosten der Sprachbreite (rund 25 statt 99 Sprachen).',
    ...
    'explainer.engines.breadcrumb': 'Whisper vs. Parakeet',
    'explainer.engines.seoTitle': 'WhisPaste: Whisper vs. Parakeet, welche lokale Engine solltest du nutzen?',
    'explainer.engines.seoDescription': 'Vergleich der zwei On-Device-Engines von WhisPaste: Whisper (whisper.cpp, 99 Sprachen, GPU-beschleunigt) und Parakeet (NVIDIA Parakeet-TDT, nur CPU, mehrfach schneller). Kostenlos und Open Source.',
    ...
    'explainer.engines.section.whisper.body': 'Whisper läuft als whisper.cpp, direkt via dart:ffi in WhisPaste eingebunden: kein Subprozess, kein Server. Es deckt 99 Sprachen und fünf Modellgrößen ab (tiny, base, small, medium, large-v3), sodass du Download-Größe und Tempo gegen Genauigkeit abwägen kannst. Auf unterstützter Hardware beschleunigt es über CUDA auf NVIDIA-GPUs oder Vulkan auf jeder modernen GPU und fällt bei einem GPU-Absturz oder fehlender GPU automatisch auf die CPU zurück.',
    ...
    'engines.whisper.desc': 'Läuft auf CUDA, Vulkan oder reiner CPU, mit fünf Modellgrößen zwischen Tempo und Genauigkeit, von tiny bis large-v3.',
    'engines.parakeet.name': 'Parakeet',

...
## 2024-05-24 - Precompiled RegExp outperforms toLowerCase in Flutter lists
**Learning:** In Dart/Flutter, repeated calls to `toLowerCase()` inside tight loops (like filtering a long list of models on every keystroke) cause high garbage collection pressure by allocating new string objects for every field checked.
**Action:** Replace `item.field.toLowerCase().contains(query)` with a precompiled `RegExp` matching the escaped query using `caseSensitive: false` outside the loop.

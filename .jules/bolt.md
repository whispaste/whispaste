...
## 2024-05-24 - Avoid `.toLowerCase()` string allocations in tight loops
**Learning:** In Dart tight loops, using `string.toLowerCase().startsWith(query)` causes massive memory allocations and GC pressure because it allocates a new lowercased string instance on every loop iteration, even for strings that will just be discarded after checking their prefix. This is particularly problematic in flutter list filtering (like search bars).
**Action:** Always prefer compiling a case-insensitive `RegExp` with a start-of-string anchor (e.g., `final regex = RegExp('^' + RegExp.escape(query), caseSensitive: false);`) *outside* the loop, and use `regex.hasMatch(string)` *inside* the loop.

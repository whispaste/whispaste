...
## 2024-05-15 - [Tight Loop Memory Optimizations]
**Learning:** Dart's `.toLowerCase()` generates unnecessary object allocations inside tight loops for large string datasets. While sometimes pre-compiling a case-insensitive `RegExp` is an easy performance win to reduce memory pressure during filtering, it should be applied specifically to cases doing prefix/substring matching (`startsWith` or `contains`) without needing locale-specific full-string lowercase conversion unless absolutely required.
**Action:** Replace tight loop `item.toLowerCase().contains(query)` or `item.toLowerCase().startsWith(query)` with a precompiled `RegExp` generated via `RegExp(RegExp.escape(query), caseSensitive: false)` placed outside the loop.

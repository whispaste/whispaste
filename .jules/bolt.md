...
## 2026-09-13 - Contextual optimization of side panel search
**Learning:** The same string allocation bottleneck identified in search filters (recompiling  inside tight loops) applies to the side panel search filter. Replacing it with a single case-insensitive `RegExp` prevents the allocation of multiple intermediate string objects for every row.
**Action:** Always evaluate loops dealing with case-insensitive string filtering in list searches to see if the dataset size warrants pulling the case-insensitivity out of the loop.
## 2023-11-20 - Contextual optimization of side panel search
**Learning:** The same string allocation bottleneck identified in search filters (calling `.toLowerCase()` inside tight loops) applies to the side panel search filter. Replacing it with a single case-insensitive `RegExp` prevents the allocation of multiple intermediate string objects for every row on every keystroke.
**Action:** When finding a tight loop with string case manipulation, evaluate if pulling the case-insensitivity out of the loop using RegExp provides a measurable performance benefit, provided it avoids repetitive string copying.

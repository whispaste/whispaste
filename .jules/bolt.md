...
## 2024-05-30 - Unicode Length Mutation in toLowerCase
**Learning:** `String.toLowerCase()` does NOT reliably preserve UTF-16 code unit lengths for all characters (e.g., Turkish 'İ' `U+0130` converts to length 2 `i\u0307` in JS/V8, though Dart's basic string conversion often flattens it to length 1). Even so, reviewers will reject code that pre-computes a lowercase string and applies token offsets derived from the *original* string to it, as this assumption leads to index out-of-bounds or misaligned substring logic for international inputs.
**Action:** Never extract substrings from a lowercased string using indices generated from the original string. When optimizing case-insensitive search logic in tight loops, use a precompiled `RegExp(pattern, caseSensitive: false)` (e.g., `RegExp('^' + RegExp.escape(query))`) to bypass `toLowerCase()` allocations safely, rather than mutating the haystack strings globally before sliding a window.

...
## 2026-12-07 - Keyboard focus on interactive text blocks
**Learning:** Interactive text blocks (like the transcript view in the history detail panel) using bare `GestureDetector` wrapped in `Semantics(button: true)` are readable by screen readers and even activatable via screen reader taps, but remain completely unreachable via keyboard Tab navigation because they lack a focusable surface.
**Action:** Replace the bare `GestureDetector` in such interactive text regions with an `InkWell` wrapped in a `WpFocusRing` passing the appropriate `FocusNode` (e.g. `editorFocusNode`), and set the `InkWell` interaction colors (focusColor, hoverColor, splashColor, highlightColor) to `Colors.transparent` so they do not conflict with the existing visual hover states.

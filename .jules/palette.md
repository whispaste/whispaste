...
## 2023-11-10 - Keyboard accessibility on Detail Panel text zones and notes headers
**Learning:** Text zones acting as edit triggers (like `_DetailPanelHeader` and `_DetailTranscriptZone` in `history_detail_panel.dart`) and section headers acting as buttons (like `_AddNoteHeaderTapTarget` in `history_notes_section.dart`) that rely on bare `GestureDetector` widgets are unreachable by keyboard navigation.
**Action:** Replace the `GestureDetector` wrappers with an `InkWell` combined with a `WpFocusRing` and a shared `FocusNode`. To maintain their original visual hover behaviors (which are usually managed by inner structures like `MouseRegion` or `AnimatedContainer`), set the `InkWell`'s interaction colors (`focusColor`, `hoverColor`, `splashColor`, `highlightColor`) to `Colors.transparent`. If the new structure is complex and contains text or siblings, use the `MergeSemantics` house idiom to prevent duplicate announcements and avoid nesting interactive semantics.

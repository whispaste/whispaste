...
## 2026-11-01 - Keyboard accessibility on markdown toolbar buttons
**Learning:** Formatting buttons in the markdown toolbar (`_ToolbarButton` in `markdown_toolbar.dart`) that relied on an `AnimatedContainer` with a bare `GestureDetector` were not accessible to keyboard users because they couldn't receive focus and be activated by Space/Enter.
**Action:** Replaced the `GestureDetector` with an `InkWell` wrapped in a `WpFocusRing` and managed a `FocusNode` to ensure keyboard focus states work seamlessly with existing hover and active states. Always set `focusColor`, `hoverColor`, `splashColor`, and `highlightColor` to `Colors.transparent` on the `InkWell` to not visually clash with the `AnimatedContainer`'s existing styling logic.

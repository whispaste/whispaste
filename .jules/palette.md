...
## 2023-11-09 - Keyboard accessibility on Markdown toolbar formatting buttons
**Learning:** Formatting buttons in the markdown toolbar (`WpMarkdownToolbar`) relied on an `AnimatedContainer` with a bare `GestureDetector`, which lacks keyboard accessibility.
**Action:** When updating Markdown toolbar custom components to support keyboard focus and activation, replace the `GestureDetector` with an `InkWell` wrapped in a `WpFocusRing` passing the same `FocusNode`. Ensure `focusColor`, `hoverColor`, `splashColor`, and `highlightColor` are set to `Colors.transparent` on the `InkWell` to not visually clash with the `AnimatedContainer`'s existing styling logic.

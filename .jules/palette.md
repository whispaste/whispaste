...
## 2026-11-20 - Keyboard focus on Markdown Toolbar buttons
**Learning:** The formatting buttons in the Markdown toolbar (`_ToolbarButton` in `markdown_toolbar.dart`) used a bare `GestureDetector` that was not accessible to keyboard users because it couldn't receive focus, breaking the "no mouse needed" promise.
**Action:** Replaced `GestureDetector` with an `InkWell` combined with a `WpFocusRing` and a shared `FocusNode`. Suppressed standard `InkWell` interaction colors (focus, hover, splash, highlight) by setting them to `Colors.transparent` to rely on the existing `AnimatedContainer` and custom focus ring visuals instead.

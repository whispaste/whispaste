...
## 2026-11-04 - Keyboard accessibility for Markdown Toolbar buttons
**Learning:** The markdown toolbar format buttons (`_ToolbarButtonState`) used a bare `GestureDetector` which cannot receive keyboard focus, meaning keyboard users could not tab to them or activate them with Enter/Space.
**Action:** Replaced `GestureDetector` with an `InkWell` wrapped in a `WpFocusRing` and added a shared `FocusNode` per button. The `InkWell`'s interaction colors were set to `Colors.transparent` to rely solely on the existing `AnimatedContainer` visual states, which were updated to include `_focusNode.hasFocus` in their highlight condition.

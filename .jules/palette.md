...
## 2026-11-14 - Keyboard focus on Insufficient RAM Screen buttons
**Learning:** Even specialized, standalone pre-flight screens like `WpInsufficientRamScreen` must be accessible. The `_QuitButton` and `_LearnMoreLink` used bare `GestureDetector` widgets wrapped in `Semantics`, making them inaccessible via keyboard navigation.
**Action:** When creating or maintaining full-screen blocking UI, always ensure interactive elements are keyboard-accessible by using the house idiom: an `InkWell` paired with a `WpFocusRing` sharing the same `FocusNode`, and setting the `InkWell` interaction colors to transparent so they do not conflict with the custom `AnimatedContainer` visual states.

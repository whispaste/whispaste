...
| Datei | Zweck |
|---|---|
| `com.whispaste.whispaste.yml` | Flatpak-Manifest (App-ID = bestehende Reverse-DNS-ID aus `linux/CMakeLists.txt`) |
| `com.whispaste.whispaste.desktop` | Desktop-Entry (Flathub-Pflicht) |
| `com.whispaste.whispaste.metainfo.xml` | AppStream-Metainfo (Flathub-Pflicht) |
...
2. **Beschreibbarer, ausführbarer Datenpfad** — `--filesystem=xdg-data/whispaste:create`.
   ...
   ausgeführt. `XDG_DATA_HOME` (`~/.var/app/com.whispaste.whispaste/data`) ist im
...
python3 -c "import yaml,sys; yaml.safe_load(open('packaging/flatpak/com.whispaste.whispaste.yml'))"

# 2. Desktop-Entry validieren:
desktop-file-validate packaging/flatpak/com.whispaste.whispaste.desktop

# 3. AppStream-Metainfo validieren:
appstreamcli validate packaging/flatpak/com.whispaste.whispaste.metainfo.xml
...
flatpak-builder --force-clean --user --install build-dir \
  packaging/flatpak/com.whispaste.whispaste.yml
flatpak run com.whispaste.whispaste
```
...
1. **Archive-Quelle pinnen** in `com.whispaste.whispaste.yml` → `sources[0]`:
   ...
   - `sha256` auf den echten Digest setzen:
     ...
     ```
2. **Release-Eintrag** in `com.whispaste.whispaste.metainfo.xml` voranstellen
...
1. Fork `github.com/flathub/flathub`, neuer Branch `com.whispaste.whispaste`.
...
`com.whispaste.whispaste` — die **bestehende** Reverse-DNS-ID aus

...
*Last updated: 2026-04-06 19:13 UTC*
...
|------|------|------|--------|---------|
| 2026-04-06 | overlay-review-fixes | Large | amboss/build-recording-pipeline | Applied 15 review findings: fixed _buildErrorCenter Flexible crash, privacy badge shows text (L10n), cancel tooltip, button accessibility (44×44), stop focusable, dynamic done message matching paste mode, overlayMode migration, hotkeyLabel safety, dead code cleanup |
| 2026-04-06 | overlay-refactor | Large | amboss/build-recording-pipeline | Major overlay rework: moved to bottom, removed green done state, removed pause button, simplified progress bar, added overlay mode setting, compact pill (480×64), stronger blur (σ=12), wired all L10n keys |
| 2026-04-05 | build-recording-pipeline | Large | amboss/build-recording-pipeline | Flutter migration WS1-WS5: settings page (57+ fields, 11 sections), recording state machine, FAB with pulse animation, onboarding overlay (4-step wizard), recording overlay (pill HUD), hotkey/sound services, recording orchestrator |
| 2026-04-04 | multi-workstream-phase1-5 | Large | main | Go app: Smart Mode async, notes redesign, sound reliability, settings reorg, floating button images |
| 2026-04-03 | fix-7-bugs | Large | main | Go app: floating button, overlay, preflight, autotag fixes |
| 2026-04-02 | fix-crash-report-noise | Large | amboss/fix-crash-report-noise → main | Crash report noise reduction, error suppression fixes |
| 2026-04-01 | crash-report-analysis | Medium | main | 23 Discord crash reports analyzed, 5 root causes fixed |

---

## Current Focus

**Flutter Migration (v2.0)** — Active on `amboss/build-recording-pipeline`:
- [x] Settings page fully implemented (57+ fields, 11 sections, no mock data)
- [x] Recording state machine + orchestrator
- [x] Recording FAB with pulse animation
- [x] Onboarding overlay (4-step wizard)
- [x] Recording overlay (pill HUD with waveform, timer, privacy badge)
- [x] Full review cycle with code + persona reviews applied
- [ ] Multi-window floating overlay (Phase B — deferred)
- [ ] Onboarding flow completion (backend wiring)
- [ ] Go FFI bridge (connect Flutter UI to Go backend)
- [ ] Cross-platform audio capture
- [ ] History page (SQLite via Drift)

## Learned Patterns

### Flutter-specific (new)
- Override StreamProviders with `Stream.value()` in tests to avoid Drift DB timer issues with pumpAndSettle
- `makeTestable()` in `test/fixtures/test_helpers.dart` wraps widgets in ProviderScope + MaterialApp with real theme
- Settings persist via SQLite (drift) in `historyDatabaseProvider.writeAppSettings()`, NOT SharedPreferences
- `settingsProvider` AsyncNotifier is the single source of truth for all app settings
- In Riverpod 3.x, use `.value` not `.valueOrNull` for nullable access on AsyncValue
- Hover transitions: asymmetric timing — WpMotion.fast (120ms) in, WpMotion.hoverOut (50ms) out
- Privacy badge labels must use L10n (`overlayProcessingLocal`/`overlayProcessingCloud`), not hardcoded strings
- `_buildErrorCenter` must NOT wrap in Flexible when inside AnimatedSwitcher (ParentDataWidget crash)
- Button touch targets must be ≥44×44px for accessibility compliance

### Go-specific (legacy, still relevant for backend)
- History metadata: `language` (user-visible) vs `language_hint` (processing hint)
- Smart Mode must run async (goroutine + Dispatch callback) to avoid UI blocking
- Sound playback on Windows: SND_SYNC on dedicated goroutine for reliability
- Config save handler must copy ALL fields — missing fields reset to defaults
